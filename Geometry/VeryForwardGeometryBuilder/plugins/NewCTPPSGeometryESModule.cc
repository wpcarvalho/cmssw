 ...
 **/
class NewCTPPSGeometryESModule : public edm::ESProducer {
public:
  NewCTPPSGeometryESModule(const edm::ParameterSet&);
  ~NewCTPPSGeometryESModule() override {}
...
private:
  ...
  const edm::ESGetToken<PDetGeomDesc, IdealGeometryRecord> gdIdealToken_;
...
NewCTPPSGeometryESModule::NewCTPPSGeometryESModule(const edm::ParameterSet& iConfig)
    : verbosity_(iConfig.getUntrackedParameter<unsigned int>("verbosity")),
      gdIdealToken_{setWhatProduced(this, &NewCTPPSGeometryESModule::produceIdealGD)
                            .consumes<PDetGeomDesc>(edm::ESInputTag(
                                "" /*optional module label */, iConfig.getParameter<std::string>("geometryIdealTag")))},
      gdRealTokens_{setWhatProduced(this, &NewCTPPSGeometryESModule::produceRealGD)},
      gdMisTokens_{setWhatProduced(this, &NewCTPPSGeometryESModule::produceMisalignedGD)},
      dgdRealToken_{
          setWhatProduced(this, &NewCTPPSGeometryESModule::produceRealTG).consumes<DetGeomDesc>(edm::ESInputTag())},
      dgdMisToken_{
          setWhatProduced(this, &NewCTPPSGeometryESModule::produceMisalignedTG).consumes<DetGeomDesc>(edm::ESInputTag())} {
}

void NewCTPPSGeometryESModule::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<unsigned int>("verbosity", 1);
  desc.add<std::string>("geometryIdealTag", std::string());
  descriptions.add("NewDoodadESSource", desc);
//  descriptions.add("DoodadESSource", desc);
}

//----------------------------------------------------------------------------------------------------

void NewCTPPSGeometryESModule::applyAlignments(const DetGeomDesc& idealGD,
...
void NewCTPPSGeometryESModule::buildDetGeomDesc(DDFilteredView* fv, DetGeomDesc* gd) {
  ...
  do {
    ...
    uint32_t detId = newGD->geographicalID();
    
    if (!(newGD->params().empty()) && (newGD->sensorType() != "")) edm::LogInfo("NewCTPPSGeometryESModule") 
      << newGD->translation().x() << " , " << newGD->translation().y() << " , " << newGD->translation().z() << " , " 
      << newGD->name() << " , " << newGD->copyno() << " , " << newGD->parentZPosition() << " , " 
      << detId << " , " << newGD->sensorType() << " , " << (newGD->params())[0] ;
//      << (newGD->geographicalID()).rawId() ;
...
std::unique_ptr<DetGeomDesc> NewCTPPSGeometryESModule::produceIdealGD(const IdealGeometryRecord& iRecord) {
...
//  edm::ESHandle<PDetGeomDesc> geometry;
//  iSetup.get<VeryForwardIdealGeometryRecord>().get(geometry);
  
//  PDetGeomDesc* pdet = geometry.product();

  auto pdet = iRecord.get(gdIdealToken_);
//  auto const& pdet = iRecord.get(gdIdealToken_);
    
  // conversion to DetGeomDesc structure
//  auto root = std::make_unique<DetGeomDesc>(&fv);
//  buildDetGeomDesc(&fv, root.get());

  auto root = std::make_unique<DetGeomDesc>(&pdet);
  
//  edm::LogInfo("NewCTPPSGeometryESModule") << "DetGeomDesc size is: " << (root->components()).size();
...
template <typename REC>
std::unique_ptr<DetGeomDesc> NewCTPPSGeometryESModule::produceGD(IdealGeometryRecord const& iIdealRec,
...
std::unique_ptr<DetGeomDesc> NewCTPPSGeometryESModule::produceRealGD(const VeryForwardRealGeometryRecord& iRecord) {
  return produceGD(iRecord.getRecord<IdealGeometryRecord>(),
                   iRecord.tryToGetRecord<RPRealAlignmentRecord>(),
                   gdRealTokens_,
                   "NewCTPPSGeometryESModule::produceRealGD");
}

//----------------------------------------------------------------------------------------------------

std::unique_ptr<DetGeomDesc> NewCTPPSGeometryESModule::produceMisalignedGD(
    const VeryForwardMisalignedGeometryRecord& iRecord) {
  return produceGD(iRecord.getRecord<IdealGeometryRecord>(),
                   iRecord.tryToGetRecord<RPMisalignedAlignmentRecord>(),
                   gdMisTokens_,
                   "NewCTPPSGeometryESModule::produceMisalignedGD");
}

//----------------------------------------------------------------------------------------------------

std::unique_ptr<CTPPSGeometry> NewCTPPSGeometryESModule::produceRealTG(const VeryForwardRealGeometryRecord& iRecord) {
...
std::unique_ptr<CTPPSGeometry> NewCTPPSGeometryESModule::produceMisalignedTG(
    const VeryForwardMisalignedGeometryRecord& iRecord) {
...
DEFINE_FWK_EVENTSETUP_MODULE(NewCTPPSGeometryESModule);
